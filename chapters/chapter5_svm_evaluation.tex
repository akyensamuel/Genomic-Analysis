...
\begin{enumerate}
    ...
    \item \textbf{Patient-pairing limitation:} the cross-validation splits used to produce Table~\ref{tab:gse19804_svm_results} are stratified by class label only and do not account for the patient-paired design of GSE19804 described in Chapter~3 (Section~\ref{sec:cv}). Because a tumour sample and its matched normal sample from the same patient can fall in different folds, the results above should be treated as an upper bound on achievable performance pending regeneration under a patient-grouped cross-validation split (e.g.\ \texttt{StratifiedGroupKFold} by patient identifier), which is identified as the more rigorous protocol for this dataset.
\end{enumerate}
