...
\midrule
Filter   & \texttt{filter\_ttest} (Welch's $t$-test) & 23,551 & 6,498 & 56.93\% & 88.12\% \\
Filter   & \texttt{filter\_anova} (ANOVA $F$-test) & 23,577 & 3,066 & 56.88\% & 94.39\% \\
Filter   & \texttt{filter\_fdr\_ranked} (BH-FDR ranked) & 19,132 & 2 & 65.01\% & $\approx$100\% \\
Wrapper  & \texttt{wrapper\_svm} (RFE to $n_{\mathrm{train}}$) & 96 & 97 & 99.82\% & 99.82\% \\
Embedded & \texttt{wrapper\_rf} (importance $>$ mean) & 273 & 318 & 99.50\% & 99.42\% \\
Embedded & \texttt{wrapper\_xgb} (XGBoost gain $>$ mean) & 41 & 45 & 99.93\% & 99.92\% \\
Embedded & \texttt{embedded\_lasso} (nonzero $L_1$) & 37 & 47 & 99.93\% & 99.91\% \\
\bottomrule
\end{tabular}
}
\end{table}
...
\midrule
Welch's $t$-test & 23,551 & 56.93\% \\
ANOVA $F$-test & 23,577 & 56.88\% \\
FDR-ranked & 19,132 & 65.01\% \\
SVM-RFE (RFE to $n_{\mathrm{train}}$) & 96 & 99.82\% \\
RF importance ($>$ mean) & 273 & 99.50\% \\
XGBoost gain ($>$ mean) & 41 & 99.93\% \\
LASSO (nonzero $L_1$) & 37 & 99.93\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Method} & \textbf{Probes retained} & \textbf{Reduction} \\
\midrule
Welch's $t$-test & 6,498 & 88.12\% \\
ANOVA $F$-test & 3,066 & 94.39\% \\
FDR-ranked & 2 & $\approx$100\% \\
SVM-RFE (RFE to $n_{\mathrm{train}}$) & 97 & 99.82\% \\
RF importance ($>$ mean) & 318 & 99.42\% \\
XGBoost gain ($>$ mean) & 45 & 99.92\% \\
LASSO (nonzero $L_1$) & 47 & 99.91\% \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Smallest retained set & 19,132 (BH-FDR) & 2 (BH-FDR) \\
Largest retained set & 23,577 (ANOVA) & 6,498 (Welch $t$-test) \\
\bottomrule
\end{tabular}
\end{table}
