...
\paragraph{Gradient Boosting (XGBoost) Importance.} Another wrapper approach uses ensemble trees like XGBoost. The model is trained on the candidate features, and the features are ranked by their gain (the improvement in accuracy brought by a feature to the branches it is on). Features whose gain importance exceeds a certain threshold (e.g., the mean importance) are retained, providing a compact and highly discriminative subset.
