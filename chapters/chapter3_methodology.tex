...
Because Affymetrix series on GEO are normalized with either MAS5 (which reports values on a linear intensity scale, typically ranging from single digits into the tens of thousands) or RMA/GCRMA (which reports values already on a $\log_2$ scale, typically ranging from about 0 to 16), the two possibilities are not interchangeable: applying a further $\log_2(x+1)$ transformation to data that is already $\log_2$-scale would compress an already-appropriately-scaled variable a second time. This was checked directly for both series by inspecting the value range of the downloaded matrix before the $\log_2(x+1)$ step described in Section~\ref{sec:preprocessing} below: values confined to approximately $[0, 16]$ indicate RMA/GCRMA-style data already on a log scale, while values extending into the hundreds or thousands indicate MAS5-style linear-scale data for which the $\log_2(x+1)$ transformation is appropriate and necessary. \textbf{[Insert the observed raw-value range for GSE19804 and GSE42568 here, together with the resulting yes/no determination for each series, once the pre-transform profiling step has been run against the downloaded files.]}
...
The classification threshold was not fixed at the default value $0.5$ for the imbalanced analyses. Instead, after a classifier was fitted on the (inner) training data, the threshold that maximised Youden's $J$ statistic was selected using \emph{out-of-fold} decision scores obtained from a nested inner cross-validation performed strictly within the outer training fold: the outer training fold was itself split (via a further stratified inner split), the classifier was fitted on the inner-training portion, and decision scores were generated for the held-out inner-validation portion. The Youden threshold was then chosen from those out-of-fold inner scores:
...
\textbf{Patient grouping on GSE19804.} As described in Section~2.1, GSE19804's tumour and adjacent-normal samples are paired within the same patient. The cross-validation results reported for GSE19804 in Chapter~5 use ordinary stratified $k$-fold splitting, which stratifies by class label only and does not guarantee that both samples from the same patient fall in the same fold; a tumour sample from one patient can therefore appear in the training fold while that patient's paired normal sample appears in the validation fold, allowing patient-specific (rather than purely class-specific) signal to leak across the split. A patient-grouped cross-validation scheme --- such as \texttt{StratifiedGroupKFold}, splitting on patient/case identifier rather than sample identifier --- would remove this leakage pathway by keeping every patient's tumour-normal pair entirely within one fold. This is identified as a methodological limitation of the GSE19804 results reported in Chapter~5: those results should be treated as an upper bound on achievable performance until they are regenerated under a patient-grouped split, since ordinary stratified splitting can only make performance look as good as or better than the patient-grouped alternative, not worse. GSE42568 does not require this adjustment, since Section~2.2 establishes that its normal samples are not matched to specific cancer patients in the same way.
