...
\textbf{Selection rule:} The analysis uses uncapped significance selection. Probes are retained when $p \leq \alpha$ ($\alpha = 0.05$), and every probe passing the criterion is kept; no top-$k$ cap is applied. This preserves the full statistical evidence instead of forcing all methods into an arbitrary fixed-size panel.
...
The analysis does not tune or select a feature-count parameter $k$. Every feature passing the active $p$-value or BH-FDR rule is retained, so introducing a $k$ sweep would be inconsistent with the uncapped selection policy. The SVM regularization parameter $C$ remains tuned within each outer training fold using an inner stratified cross-validation search over $\{0.01, 0.1, 1, 10, 100\}$, while comparing class-weight settings of either \texttt{None} or \texttt{"balanced"}. The held-out outer fold is never used to choose $C$.
