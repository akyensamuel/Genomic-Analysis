...
where $\mu$ and $\sigma$ denote the feature mean and standard deviation. Standardization is particularly important for regularized methods such as LASSO and for SVMs, because it prevents features with large absolute values from dominating the optimization objective. \textbf{Fold locality:} to avoid information leakage from the held-out fold into the training statistics, $\mu$ and $\sigma$ must be estimated only from the training-fold samples within each cross-validation split, and the resulting scaler then applied unchanged to the corresponding validation fold. The scaler is therefore placed inside the same \texttt{sklearn.pipeline.Pipeline} as the feature selector and the classifier (Section~\ref{sec:cv}), rather than being fit once on the full dataset before cross-validation. Any earlier version of the pipeline that standardized the full dataset before splitting into folds should be corrected to this fold-local form, and the corrected pipeline is what generates the results reported in Chapters~4 and~5.
...
\textbf{Caution on in-sample threshold selection.} If the decision scores used to choose the Youden-optimal threshold are computed on the same training-fold observations used to fit the classifier, the threshold is chosen in-sample and can itself overfit, inflating the apparent benefit of threshold tuning. A methodologically stronger design selects the threshold using either an inner cross-validation loop nested inside the training fold, or out-of-fold predictions obtained by further splitting the training fold, so that no observation contributes to both fitting the classifier and choosing the threshold applied to it. Whichever design was actually used in the implementation should be confirmed and stated explicitly here, since the current wording (``training-fold predictions'') does not make this unambiguous.
